...
out_fname= 'out.html'
metrics_fname='metrics_2D.jsonl'
set_matplotlib_params()

recs = list(voxnet.metrics_logging.read_records(metrics_fname))
...
with open(out_fname, 'w') as page:
    ...
    fig = pl.figure()
    loss.plot(label='raw')
    loss.rolling(smoothing_window).mean().plot(label='smoothed')
    pl.xlabel('Iter')
    pl.ylabel('Loss')
    pl.legend()
    ...
    fig = pl.figure()
    acc.plot(label='raw')
    acc.rolling(smoothing_window).mean().plot(label='smoothed')
    pl.xlabel('Iter')
    pl.ylabel('Accuracy')
    fig.tight_layout(pad=0.1)
    sio = StringIO.StringIO()
